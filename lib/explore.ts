...
export type ExploreSort = 'newest' | 'most_supported'
...
export function parseExploreProjectsQuery(args: {
  ...
  let sort: ExploreSort = 'newest'
  if (args.rawSort && args.rawSort !== '') {
    if (args.rawSort !== 'newest' && args.rawSort !== 'most_supported') {
      return { ok: false, error: 'sort must be newest or most_supported' }
    }
    sort = args.rawSort
  }
